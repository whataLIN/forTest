...
if choice == "메인페이지":
    ...
    for i in range(5):
        ...
        while total_stats <= 40:
            ...
            player["Shooting"] = st.slider(f"슈팅", min_value=1, max_value=10, value=1, key=f"shooting_{i}")
            player["Dribbling"] = st.slider(f"드리블", min_value=1, max_value=10, value=1, key=f"Dribbling_{i}")
            player["Passing"] = st.slider(f"패스", min_value=1, max_value=10, value=1), key=f"Passing_{i}"
            player["Rebounding"] = st.slider(f"리바운드", min_value=1, max_value=10, value=1, key=f"Rebounding_{i}")
            player["Defense"] = st.slider(f"수비", min_value=1, max_value=10, value=1, key=f"Defense_{i}")
            player["Stamina"] = st.slider(f"스테미나", min_value=1, max_value=10, value=1, key=f"Stamina_{i}")
